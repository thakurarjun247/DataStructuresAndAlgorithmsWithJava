...
public class Recursion {
    ...
    public char[] reverseArray(char[] array, int i) {
        ...
        //time and space complexities O(n), O(1)
        // swap first and last element of datastructures.array using temp var
        // here i=0
        char temp;
        temp = array[i];
        array[i] = array[array.length - 1 - i];
    ...
    public char[] reverseArrayNew(char[] array, int i) {
        ...
        if (i >= array.length / 2) { // this if can be written as below as well if(i>=datastructures.array.length-1-i)
            return array;
        }
        char temp = array[i];
        array[i] = array[array.length - 1 - i];
