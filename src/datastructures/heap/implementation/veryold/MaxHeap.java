package datastructures.heap.implementation.veryold;
...
public class MaxHeap {
    public static void main(String[] s) {
        ...
        //datastructures.heap.heapSort(list);
        System.out.println(list);
...
class MaxHeapUtil {
    ...
    int heapExtractMax(List<Integer> list) {
        if (list.isEmpty())
            throw new NoSuchElementException("empty datastructures.heap, can't extract max");
        Integer max = list.get(0);
        int lastIndex = list.size() - 1;
        list.set(0, list.get(lastIndex));
        list.remove(lastIndex);
        maxHeapyfy(list, 0);//settle down
        return max;
